...
class RandomMove(object):
    def __init__(self, move=0.2):
        self.move = move

    def __call__(self, sample):
        move_x = self.move * random.random()
        move_y = self.move * random.random()
        for elem in sample.keys():
            tmp = sample[elem]
            h, w = tmp.shape[:2]
            M = np.float32([[1, 0, move_x * w], [0, 1, move_y * h]])
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        for elem in sample.keys():
            ...
            sample[elem] = torch.from_numpy(tmp.copy())
